...
/* ── График закрытой сделки ──────────────────────────────────────────────────
   ...
   Окно: ВОСЕМЬ ЧАСОВ ДО ВХОДА и восемь часов после закрытия (решение
   владельца 23.09.2026). Раньше слева бралось 15% длины окна, и разбирать было
   нечего: сделка контура живёт десятки минут, значит слева оставалось пять
   свечей — график начинался уже внутри сетапа.

   А сетап целиком лежит ИМЕННО СЛЕВА. Контур строит его на часовом ATR, EMA50
   трёх таймфреймов и границах движения по пятиминуткам: «шорт в памп» без
   самого пампа на экране — это просто свеча, от которой мы почему-то
   продались. Восемь часов выбраны не на глаз: ровно столько меряет дозамер
   ПОСЛЕ сигнала, и одинаковый горизонт с обеих сторон делает картинку
   симметричной — видно и то, из чего мы вошли, и то, что получилось. */
const BEFORE_MS = 8 * 3600e3;
const AFTER_MS = 8 * 3600e3;

/** Шаг свечей под длину окна: цель — 250 бар. Было 110, но с восемью часами
 *  слева окно уже НИКОГДА не короче шестнадцати часов, и на прежней цели
 *  график молча переехал бы на пятнадцатиминутки — то есть мы добавили бы
 *  истории и тут же потеряли бы детализацию, ради которой её добавляли.
 *
 *  250 держит на пяти минутах всё окно до 20.8 часа, то есть любую сделку
 *  контура (они живут минуты и часы) — ровно тот масштаб, который владелец
 *  просил везде. Дальше шаг растёт сам: двухдневная сделка на пятиминутках
 *  дала бы 600 волосков вместо графика. */
function intervalFor(ms: number): Interval {
  const want = ms / 250 / 1000;
    ...
    <>
      ...
      <div className="flex items-center justify-between mt-1.5">
        <span className="text-[10px]" style={{ color: "var(--label-3)" }}>
          {INTERVALS.find((i) => i.id === iv)?.label} · 8 часов до входа, сделка и 8 после
        </span>
